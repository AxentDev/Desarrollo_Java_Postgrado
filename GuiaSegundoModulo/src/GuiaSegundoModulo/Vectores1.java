package GuiaSegundoModulo;

public class Vectores1 //Los elementos de un vector o array se empiezan a numerar en el 0 y permiten gestionar desde una sola variable múltiples datos del mismo tipo.
{
	public static void main(String args[])
	{
		//Forma1:
		int vectorNumeros1[] = new int[10];
		//Forma 2:
		int []vectorNumeros2 = new int[10];
		//Forma 3:
		int[] vectorNumeros3 = new int[10];
		//Forma 4:
		int vectorNumeros4[];
		//Forma 5:
		int vectorNumeros5[] = {};
		//Forma 6:
		int vectorNumeros6[] = {9,8,7,6,5,4,3,2,1,0};
		//Forma 7:
		int vectorNumeros7[] = new int[] {9,8,7,6,5,4,3,2,1,0};
	}

}
